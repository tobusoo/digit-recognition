    ...
    for (size_t i = 0; i < size; i++) {
        for (size_t j = 0; j < size; j++) {
            ...
            if (rect.contains(mouse_pos.x, mouse_pos.y)) {
                ...
                input_nn.array[j * size + i] = color_to_double(color);

                if (i > 0) {
                    color = v[i - 1][j].getFillColor();
                    color = change_color(color, true);
                    v[i - 1][j].setFillColor(color);
                    input_nn.array[j * size + (i - 1)] = color_to_double(color);
                }
                if (i < size - 1) {
                    color = v[i + 1][j].getFillColor();
                    color = change_color(color, true);
                ...
                if (j > 0) {
                    ...
                    input_nn.array[(j - 1) * size + i] = color_to_double(color);
                }
                if (j < size - 1) {
                    color = v[i][j + 1].getFillColor();
                    color = change_color(color, true);
...
void clean_field(sf::RectangleShape v[][28], tbs::Matrix input_nn, size_t size)
{
    input_nn.fill(0);
    for (size_t i = 0; i < size; i++) {
        for (size_t j = 0; j < size; j++) {
...
int main()
{
    ...
    tbs::Matrix input_nn(1, size * size);
    ...
    while (window.isOpen()) {
        ...
        nn->forward(input_nn);
        const tbs::Matrix* out = nn->output();
        double max = out->array[0];
        int max_i = 0;
        ...
        for (size_t i = 0; i < out->cols; i++) {
            ...
            ImGui::GetStyle().Colors[ImGuiCol_PlotHistogram]
                    = GetProgressBarColor(out->array[i]);
            ImGui::ProgressBar(
                    out->array[i], {-1, 0}, std::to_string(i).c_str());
            ImGui::GetStyle().Colors[ImGuiCol_PlotHistogram]
                    = originalFillColor;
            if (out->array[i] > max) {
                max = out->array[i];
